...
selection="$(
  "$session_order" list "$sesh_bin" -t --icons | fzf \
    --ansi \
    --disabled \
    --layout=reverse \
    --cycle \
    ...
    --bind 'esc:abort' \
    --bind "change:transform:[[ -z \"\$FZF_QUERY\" ]] && echo \"reload($session_order list $sesh_bin -t --icons)+disable-search+change-prompt(Nav ›  )+rebind(j,k,q,/)\"" \
